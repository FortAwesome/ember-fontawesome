...
function buildConditional(b, conditionArgs) {
  let args = conditionArgs.map((p) =>
    typeof p === 'string' ? b.string(p) : p
  );
...
function isDynamic(node) {
  return (
    (node.type !== undefined && node.type === 'PathExpression') ||
...
function interpolateSize(component) {
  if (component.invocationAttrs.size) {
    if (isDynamic(component.invocationAttrs.size)) {
      return buildConcatIfPresent(
        component._syntax.builders,
        component.invocationAttrs.size,
        ['fa-', component.invocationAttrs.size, 'x']
      );

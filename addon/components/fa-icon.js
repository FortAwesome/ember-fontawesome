...
function classList() {
  let classes = {
    ...
    [`fa-rotate-${this.rotation}`]:
      getWithDefault(this, 'rotation', null) !== null,
...
const IconComponent = Component.extend({
  ...
  html: computed('abstractIcon.children.[]', function () {
    ...
    if (!abstractIcon) {
      ...
      newHtml = htmlSafe(
        abstractIcon.children.reduce((acc, cur) => {
  ...
  abstractIcon: computed(
    ...
    function () {
      ...
      if (!renderedIcon) {
        console.warn(
          `Could not find icon: iconName=${iconLookup.iconName}, prefix=${iconLookup.prefix}`
        );
        return null;
      }
